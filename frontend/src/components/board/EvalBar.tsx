...
interface EvalBarProps {
  eval_score: number; // in pawns, from white's perspective
  mate_in?: number | null;
  height?: number;
}

export function EvalBar({ eval_score, mate_in, height = 560 }: EvalBarProps) {
  // Calculate white's percentage (50% = equal, 100% = white winning fully)
  ...
  if (mate_in !== null && mate_in !== undefined) {
    ...
    // Sigmoid-like mapping: clamp eval to visual range
    // ±5 pawns maps to roughly 10%-90%
    const sigmoid = 1 / (1 + Math.exp(-eval_score * 0.5));
    whitePercent = sigmoid * 100;
    const absEval = Math.abs(eval_score);
    displayText = absEval >= 10 ? `${eval_score > 0 ? '+' : ''}${eval_score.toFixed(0)}` : `${eval_score > 0 ? '+' : ''}${eval_score.toFixed(1)}`;
  }
